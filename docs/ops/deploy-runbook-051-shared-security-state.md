...
`scripts/deploy-backend.sh` ทำแค่ `git pull` → ตรวจ syntax → รัน unit test → `pm2 reload` *(อัปเดต 5 ก.ย. 2569 ค่ำ: ตั้งแต่ commit ที่แก้สคริปต์หลัง `d889530` ขั้น pull เป็น fetch + `--ff-only` ที่หยุดเมื่อล้ม และมี `npm ci` อัตโนมัติเมื่อ lockfile เปลี่ยน — ยังไม่รัน migration เหมือนเดิม)*
