...
| 3 | **A4-2a** | — | A | **Dry run ก่อนลงนาม:** รันชุด §12 ของ master plan ด้วย `--allow-pending` แล้วพิสูจน์ว่างานที่เหลือ **รอเฉพาะการลงนามและการปรับ scorecard** ไม่มีงานเทคนิคหรือ evidence pack ที่ยังไม่มี | `owner-actions.json` มี `fail = 0` และทุกแถวที่เหลือเป็น human gate ตาม §14 เท่านั้น: `signoff-*`, `approval-*` และ readiness verifier ของ evidence pack ที่คนต้องส่ง (`readiness-operator-gate-evidence`, `readiness-phase9-evidence`, `readiness-restore-drill-evidence`, `readiness-uat-evidence-pack`, `readiness-uat-evidence-safety`, `readiness-go-live-signoff`, `readiness-scorecard-overall` — ทั้งเจ็ดมี `category = readiness-verifier` ตาม schema ของ `create-go-live-bundle.js` ไม่ใช่ `signoff`) — ดู §13 สำหรับจำนวนที่รันได้จริง |
...
| A4-2a | - | `node scripts/create-go-live-bundle.js --allow-pending` → `node scripts/validate-go-live-bundle.js <bundle> --allow-pending` → `node scripts/summarize-go-live-closure.js --bundle <bundle> --allow-pending` → `node scripts/validate-go-live-closure-status.js <closure> --allow-pending` | `owner-actions.json` | ทั้ง 4 คำสั่ง exit 0 และ `fail=0`; ทุก `id` ที่เหลือต้องเป็น human gate ตาม §14 เท่านั้น — รันจริงเมื่อ 5 ก.ย. 2569 ได้ 20 action ใน 3 ตระกูล: `signoff-*` (8), `approval-*` (5) และ readiness verifier ของ evidence pack ที่คนต้องส่ง (7) คือ `readiness-operator-gate-evidence`, `readiness-phase9-evidence`, `readiness-restore-drill-evidence`, `readiness-uat-evidence-pack`, `readiness-uat-evidence-safety`, `readiness-go-live-signoff`, `readiness-scorecard-overall`. **เกณฑ์เดิมระบุไว้แค่ 4 ชื่อ** จึงทำให้ผลรันที่ถูกต้องอ่านเป็น "มี id นอกชุด" — ที่ผิดคือเอกสาร ไม่ใช่ tooling |
