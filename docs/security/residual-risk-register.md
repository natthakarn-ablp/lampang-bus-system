...
| ตำแหน่งในโค้ด (ที่ `a0e783e`) | **ปิดแล้ว** — `affiliation.routes.js:552` ผูก `exportFormatLimiter` กับ `GET /audit-logs` แล้ว (สาขา CSV `:586`) และไฟล์ import ทั้ง `importExportLimiter` และ `exportFormatLimiter` (`:11`) — **อัปเดต 2026-09-06:** มี source-level scan `backend/tests/exportRateLimitCoverage.unit.test.js` ยืนยันว่าทุก route ที่สร้าง export (ตรวจจาก `Content-Disposition`/`format === 'csv'` ฯลฯ ใน handler) ถูกประกาศพร้อม `exportFormatLimiter`/`importExportLimiter` และ route `/audit-logs` ทั้ง 4 บทบาทใช้ limiter เดียวกัน — เป็น **หลักฐานระดับ source ไม่ใช่ runtime** (limiter ทุกตัว `skip` ใต้ `NODE_ENV=test` จึงทดสอบแบบยิงคำขอแล้วรอ 429 ไม่ได้) RR-04 จึงปิดในโค้ดและมีเทสต์กัน regression ระดับ source แล้ว |
...
| เหตุที่ยังเปิด | **แก้ในโค้ดแล้ว 2026-09-05 แต่ยังไม่ปิด** — สามในสี่ก้อนย้ายเข้า DB แล้วผ่าน migration 051 (`login_lockouts`, `line_webhook_events_seen`, `line_bind_lockouts`) และไม่มี `Map` ที่เป็นต้นทางความจริงเหลืออยู่ในสามจุดนั้น · ที่ยังเปิดคือ (ก) `linkingState` ยังอยู่ในหน่วยความจำเพราะตารางของมันเก็บเบอร์โทรแบบอ่านได้ ซึ่งรอ DPO ตัดสินใน **D0-8** (ข) เกณฑ์ปิด A1-9 ข้อ 5 กำหนดว่าต้องมีผล capacity จาก environment ที่ใช้ปิด Phase 9 gate ซึ่งรอ **B2-2** — ผลจาก local staging (A1-8, 2026-09-05) ใช้แทนไม่ได้เพราะ scenario `login` วัดไม่ได้บน local (`loginLimiter` นับต่อ IP) |
...
| ความเสี่ยงคงเหลือหลังมาตรการ | สำหรับสามก้อนที่ย้ายแล้ว: เหลือเพียงต้นทุน 1 read ต่อการ login และ 1 write ต่อการ login ที่ล้มเหลว ซึ่งยังไม่ได้วัดบน environment จริง (ดู B2-2) · สำหรับ `linkingState` ที่ยังไม่ย้าย: ถ้า scale เป็นหลาย instance ผู้ใช้ที่กำลังผูกบัญชี LINE ค้างอยู่อาจต้องเริ่มขั้นตอนใหม่เมื่อข้อความถัดไปไปลงคนละ instance — เป็นการสะดุดของผู้ใช้ ไม่ใช่การอ่อนลงของมาตรการความปลอดภัย |
| ผู้ต้องรับความเสี่ยง | Technical owner + Operator (ชื่อจริง **รอ C0-7**) |
| ระดับความรุนแรงตาม severity scheme | **รอ C0-13** |
| สถานะ | **แก้บางส่วนแล้ว · ยังเปิด** — โค้ดสามในสี่จุดเสร็จและมีเทสต์พิสูจน์ว่าการนับข้ามการ restart ได้ (`tests/sharedSecurityState.test.js`, `tests/lineBindGuard.test.js`) ที่เหลือรอ D0-8 (DPO) และ B2-2 (capacity บน environment จริง) |

---

### RR-09 — `parentLimiter` นับ 60 คำขอ/นาที ต่อ IP ซึ่งผู้ปกครองทั้งเครือข่ายใช้ร่วมกัน

| หัวข้อ | เนื้อหา |
|---|---|
| คำอธิบาย | `/api/parent/*` ทั้ง router ถูกจำกัดที่ 60 คำขอต่อนาที **ต่อ IP** ผู้ปกครองเข้าผ่าน LIFF บนมือถือ ผู้ใช้ที่ออกอินเทอร์เน็ตผ่าน CGNAT ของเครือข่ายมือถือเดียวกัน หรือผ่าน wifi โรงเรียนเดียวกัน จะถูกนับรวมเป็น IP เดียว เพดานนี้จึงเป็นเพดานของทั้งกลุ่ม ไม่ใช่ของผู้ใช้คนเดียว |
| ตำแหน่งในโค้ด | `backend/src/routes/parent.routes.js:31-38` (`windowMs: 60 * 1000, max: 60`, `router.use(parentLimiter)`) — และต่างจาก limiter อื่นเกือบทุกตัวในระบบตรงที่ **ไม่มี** `skip: () => process.env.NODE_ENV === 'test'` |
| ที่มาของการค้นพบ | สังเกตจากผลจริงของ load test บน local staging 2026-09-05 (`docs/performance/load-test-local-2026-09-05.md` §2.3): scenario `parent_status` ได้ 401 ไป 60 ครั้งแล้วเป็น 429 ล้วนอีก 36,545 ครั้งภายในหนึ่งนาที — เพดานเต็มทันที ไม่ใช่การอนุมานจากการอ่านโค้ด |
| เหตุที่ยังเปิด | การเลือกว่าจะคีย์ตาม LINE user id แทน IP หรือยกเพดาน ต้องใช้ตัวเลขจำนวนผู้ปกครองจริงต่อโรงเรียนและรูปแบบการเข้าใช้ตอนเช้า ซึ่งยังไม่มี — และการคีย์ตาม LINE user id ย้ายจุดที่เชื่อถือได้จาก network layer ไปที่ id_token ซึ่งเป็นการตัดสินใจเชิงสถาปัตยกรรม ไม่ใช่การปรับค่า |
| มาตรการที่มีอยู่ | เพดานทำงานอยู่จริงและปฏิเสธอย่างถูกต้อง (429 พร้อมข้อความไทย) — ปัญหาคือความพร้อมใช้ ไม่ใช่ความปลอดภัย ไม่มีทางที่ผู้ใช้จะเห็นข้อมูลของคนอื่นจากข้อนี้ |
| ความเสี่ยงคงเหลือหลังมาตรการ | ช่วงเวลาที่ผู้ปกครองเปิดดูพร้อมกันมากที่สุด (ราว 07:00 และเวลาเลิกเรียน) คือช่วงที่มีโอกาสชนเพดานสูงสุด ผู้ปกครองจะเห็น "Too many requests" ในนาทีที่ต้องการดูสถานะบุตรหลานที่สุด **ยังไม่ได้วัดว่าเกิดจริงหรือไม่บน production** — ตัวเลขผู้ใช้พร้อมกันจริงยังไม่ทราบ |
| ผู้ต้องรับความเสี่ยง | Technical owner + Operator (ชื่อจริง **รอ C0-7**) |
| ระดับความรุนแรงตาม severity scheme | **รอ C0-13** |
| สถานะ | **รอ owner ตัดสิน** (บันทึกไว้เพื่อไม่ให้ตกหล่น) |
...
### RR-11 — Cloudflare แทรกสคริปต์เก็บสถิติของบุคคลที่สามเข้าทุกหน้า และ CSP ของเราบล็อกไว้อยู่

| หัวข้อ | เนื้อหา |
|---|---|
| คำอธิบาย | ตรวจเว็บจริงด้วยเบราว์เซอร์เมื่อ 8 ก.ย. 2569 ทุกหน้าที่เป็น SPA พยายามโหลด `https://static.cloudflareinsights.com/beacon.min.js/...` แล้วถูก CSP ของเราเองปฏิเสธ เกิดข้อความ error ในคอนโซลทุกครั้งที่เปิดหน้า สคริปต์นี้ **ไม่ได้อยู่ในโค้ดของเรา** — `grep -rl cloudflareinsights frontend/ backend/` ไม่พบ และไฟล์ `index.html` ที่เราสร้างไม่มี — แต่ถูกแทรกโดย Cloudflare ที่วางอยู่หน้าเว็บ (`Server: cloudflare`, มี `CF-RAY`) ซึ่งเป็นการตั้งค่าฝั่งบัญชี Cloudflare ไม่ใช่ฝั่ง repository |
| หน้าที่ได้รับผลกระทบ | หน้าเข้าสู่ระบบ · หน้าผูกบัญชีผู้ปกครอง · หน้ากู้รหัสผ่านผู้ดูแลระบบ (หน้าที่เป็นไฟล์คู่มือแบบ static ไม่พบปัญหา) |
| ผลกระทบตอนนี้ | **ยังไม่มีข้อมูลรั่ว** เพราะ CSP บล็อกสำเร็จ สคริปต์ไม่เคยทำงาน · ผลที่เกิดจริงคือคอนโซลมี error ทุกหน้า ซึ่งกลบ error จริงที่ควรสังเกตเห็น |
| ความเสี่ยงที่แท้จริง | คือ **การแก้ผิดทาง** ถ้ามีใครเห็น error นี้แล้ว "แก้" ด้วยการเติม `https://static.cloudflareinsights.com` เข้าไปใน `script-src` ระบบจะเริ่มส่งข้อมูลการเปิดหน้าของผู้ใช้ — รวมถึงหน้าที่เกี่ยวกับข้อมูลเด็ก — ออกไปยังผู้ประมวลผลภายนอกที่หน่วยงานไม่มีข้อตกลงด้วย โดยไม่มีใครตั้งใจให้เกิด |
| ทางแก้ที่ถูกต้อง | ปิด Browser Insights / Web Analytics ในแดชบอร์ดของ Cloudflare เพื่อให้เลิกแทรกสคริปต์ตั้งแต่ต้นทาง **ห้ามแก้ด้วยการขยาย CSP** เว้นแต่ DPO ตัดสินว่าการส่งข้อมูลนี้ออกไปมีฐานทางกฎหมายรองรับ |
| ตำแหน่งในโค้ด | `frontend/index.html` (meta `Content-Security-Policy`) — โค้ดฝั่งเราถูกต้องแล้ว ไม่ต้องแก้ |
| ที่มาของการค้นพบ | ตรวจหน้าเว็บจริง 6 หน้าด้วย Playwright headless เมื่อ 8 ก.ย. 2569 พบ 3 หน้ามี error นี้ |
| ผู้ต้องรับความเสี่ยง | ผู้ดูแลบัญชี Cloudflare (ชื่อจริง **รอ C0-7**) ร่วมกับ DPO |
| ระดับความรุนแรงตาม severity scheme | **รอ C0-13** |
| สถานะ | **รอ owner ตัดสิน** — ปิดที่ Cloudflare หรือขอความเห็น DPO |
...
| การทดสอบ rate limit ของ export | ~~ค้นหา `exportFormatLimiter` และ `importExportLimiter` ใน `backend/tests` ไม่พบผลลัพธ์~~ **อัปเดต 2026-09-06:** `backend/tests/exportRateLimitCoverage.unit.test.js` เป็น source-level scan ที่อ่าน route declaration ทุกไฟล์ใน `src/routes` แล้ว assert ว่าทุก endpoint ที่สร้าง export ประกาศพร้อม `exportFormatLimiter`/`importExportLimiter` (และ `/audit-logs` ทั้ง 4 บทบาทเหมือนกัน) — ยืนยัน**รายตัว**ระดับ source; ไม่ใช่การทดสอบแบบยิงคำขอเพราะ limiter `skip` ใต้ `NODE_ENV=test` · ที่มีเดิมคือ `backend/tests/appRateLimitOrder.unit.test.js:24-93` ซึ่งตรวจการต่อ limiter **ระดับ mount** (รายการ prefix ของ global limiter, ลำดับ mount ที่ต้องอยู่ก่อน router ที่มันคุ้มกัน และ `/api/reports` ต้องมี limiter เฉพาะของตัวเอง) |
...
| RR-11 | ☐ ยอมรับ ☐ ไม่ยอมรับ ☐ ให้แก้ก่อน | | | | |
