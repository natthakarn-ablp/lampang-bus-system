...
สถานะเอกสาร: **ข้อเสนอ ยังไม่ได้ apply** — ไม่มี migration ในชุดนี้ และไม่มีการแก้ schema ใด ๆ
ผู้อนุมัติที่ต้องการ: Technical owner (schema) + Operator (หน้าต่างเวลา migration)
...
## 1. ทำไมถึงยังไม่แก้ในชุดนี้

A1-9 ระบุผลลัพธ์ว่า "ย้าย in-memory state ไป DB หรือ Redis" ซึ่งต้องสร้างตารางใหม่ กติกาของงานชุดนี้คือ **ห้ามเพิ่ม migration หรือแก้ schema เอง ให้เขียนเป็นข้อเสนอพร้อม DDL แทน** เอกสารนี้จึงเป็นส่วนที่ทำได้ ไม่ใช่การเลื่อนงาน
