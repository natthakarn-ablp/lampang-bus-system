...
export default function AffiliationLiveVehicles() {
  ...
  const fetchOnce = useCallback(async () => {
    try {
      ...
      setError(err?.response?.data?.message || 'โหลดข้อมูลตำแหน่งรถในสังกัดไม่สำเร็จ');
    }
  }, []);

  // Initial fetch + 15s polling with cancellation flag.
  // Backend will write audit_logs.action='VIEW' on first hit (5-min dedup).
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-5">
      ...
      {/* KPI strip — affiliation gets two extra cards (total + students-in-scope) */}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] gap-4">
          {/* Left: vehicle list (with student_count_in_scope per row) */}
