...
/* ── Edit-students modal: replace the student list of an existing point ── */
function EditStudentsModal({ point, role, onClose, onSaved }) {
  const apiBase = role === 'school' ? '/school' : '/driver';
  const [students, setStudents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectedIds, setSelectedIds] = useState(new Set());
  ...
  // On mount: fetch the assignable list. Pre-check students currently
  // on this point.
  useEffect(() => {
    let cancelled = false;
