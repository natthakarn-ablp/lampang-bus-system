...
export default function ProvDailyStatus() {
  ...
  // Phase 9.11 — flatten the aff→school→vehicles tree for plate
  // autocomplete. Each suggestion carries plate_no (primary) + school_name
  // (secondary) so PlateSearchInput's existing metaText() picks the right
  // label without exposing affiliation_id / vehicle_id.
  const vehicleSuggestions = useMemo(() => {
    if (!data?.affiliations) return [];
    return data.affiliations.flatMap((aff) =>
      (aff.schools || []).flatMap((s) =>
        (s.vehicles || []).map((v) => ({
          plate_no: v.plate_no,
  ...
  return (
    <div className="p-3 sm:p-6 max-w-5xl mx-auto">
      <PageHeader
        title="สถานะวันนี้"
        subtitle="ความคืบหน้าการรับ-ส่งนักเรียนรายคันทั้งจังหวัด"
        meta={data?.date
      ...
      {loading ? (
        ...
        <EmptyState title="ยังไม่มีข้อมูล" description="ยังไม่มีสังกัดที่มีข้อมูลสถานะวันนี้" />
      ) : (
