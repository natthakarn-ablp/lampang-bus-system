...
run_local_mode() {
  ...
  run_check "node syntax: readiness helpers" bash -c "cd '$ROOT' && for file in scripts/create-uat-evidence-pack.js scripts/validate-uat-evidence-pack.js scripts/summarize-uat-evidence.js scripts/scan-uat-evidence-safety.js scripts/create-go-live-signoff-draft.js scripts/create-restore-drill-evidence-pack.js scripts/validate-restore-drill-evidence.js scripts/validate-go-live-signoff.js scripts/verify-100-readiness.js scripts/create-go-live-bundle.js scripts/validate-go-live-bundle.js; do node --check \"\$file\" >/dev/null || exit 1; done"
}
